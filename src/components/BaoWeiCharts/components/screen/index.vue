<template>
  <!-- 筛选 -->
  <div>
    <el-dialog class="screenForm" title="筛选项配置信息" :append-to-body="true" :visible.sync="isShow">
      <div class="screenBox">
        <el-form ref="form">
          <el-row type="flex" class="row-bg theme-border-color" v-for="(item,index) in screenData" :key="index">
            <el-col :span="5">
              <el-form-item label="表单类型" label-width="65px">
                <el-select
            ...
            <el-col :span="7">
              <el-row type="flex">
                <el-col :span="12">
                  <el-form-item label="字段名" label-width="55px">
                    <el-input v-model="item.key" size="small"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
            ...
            <el-col :span="5">
              <el-form-item label="下级筛选引用" label-width="90px">
                <el-select v-model="item.sfxjcx" size="small" placeholder="请选择">
            ...
            <el-col :span="5">
              <el-form-item v-if="!item.dataUrl&&!item.arr" label-width="70px" label="数据配置">
                <el-input
                  :disabled="['input','number','textarea'].indexOf(item.type)>-1"
                  readonly
                  @click.native="itemDataChange(item,index)"
                  size="small"
                ></el-input>
              </el-form-item>
              <el-form-item v-if="item.dataUrl" label-width="70px" label="数据接口">
                <el-input
                  readonly
              ...
              <el-form-item v-if="item.arr" label-width="70px" label="选择数据">
            ...
            <el-col :span="2">
              <div class="rightIcon">
                ...
                ></i>
                <i
                  @click="sortNext(item,index,screenData.length-1==index)"
                  :class="['iconfont','iconxiayi','theme-color',{'disabled':screenData.length-1==index}]"
                ></i>
                <i @click="screenDelete(item,index)" class="el-icon-delete"></i>
              </div>
            </el-col>
          </el-row>
        </el-form>
      ...
      <span slot="footer" class="dialog-footer">
        <div>
          <el-button @click="addScreen" type="primary">新 增</el-button>
        </div>
        <div class="right">
    ...
    <settingData ref="settingData" @itemDataConfig="itemDataConfig"></settingData>
  </div>
</template>
<script>
