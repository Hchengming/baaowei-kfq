<template>
  <div :class="['staticWhere',{'active':isShow==true}]">
    <el-form ref="form" :model="whereAll.form" label-width="70px">
      <el-form-item v-for="(item,index) in whereAll.data" :key="index" :label="item.label">
        <!-- 输入框 -->
        <el-input v-if="item.type=='input'" v-model="whereAll.form[item.key]" size="small"></el-input>
        <!-- 下拉框 -->
        <el-select
        ...
        <!-- 单选框 -->
        <el-radio-group v-if="item.type=='radio'" v-model="whereAll.form[item.key]">
          <el-radio
            size="small"
        ...
        <el-checkbox-group v-if="item.type=='checkbox'" v-model="whereAll.form[item.key]">
          <el-checkbox
        ...
        <!-- 数字输入框 -->
        <el-input-number v-if="item.type=='number'" size="small" v-model="whereAll.form[item.key]"></el-input-number>
        <!-- 多行文本框 -->
        <el-input
          type="textarea"
          size="small"
    ...
    <treeModel ref="treeModel" @treeModelSubmit="treeModelSubmit"></treeModel>
