  ...
  <div>
    ...
    >
      <div class="map_content">
        <!-- 国家简易行政区图组件 -->
        <map1 v-if="settingForm.myType=='XZQMap'" ref="XZQ_map"></map1>
        <span class="map_setting">
          <i @click="settingClick" title="设置" class="el-icon-setting"></i>
          <i title="拖拽" @mousedown="mousedown_tz" class="icontuozhuai iconfont"></i>
          <i title="缩放" @mousedown="mousedown_ls" class="iconfont iconkuozhan"></i>
        </span>
      </div>
    </div>
    <!-- 编辑模块 -->
    <edmit ref="edmit" :mapAll="mapAll" @submit="submit" :form="settingForm"></edmit>
  </div>
</template>
<script>
import "./map.css";
