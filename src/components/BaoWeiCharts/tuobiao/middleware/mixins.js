...
export default {
    ...
    methods: {
        ...
        updateMoule(contentAreaConfig, moduleId, fn) {
            axios.post(this.settingConfig.commonUrl + "/page/updateModule", { contentAreaConfig, moduleId }).then(res => {
        ...
        deleteMoule(moduleId) {
            axios.post(this.settingConfig.commonUrl + "/page/removeModule", { moduleId }).then(res => {
                // console.log(res)
                let status = res.data.status;
        ...
        //子页面新增事件
        childSettingAdd(obj) {
            axios.post(this.settingConfig.commonUrl + "/child/addChildModule", { contentAreaConfig: obj.contentAreaConfig, parentModuleId: obj.moduleId, key: obj.key }).then(res => {
                // console.log(res)
                let status = res.data.status;
                // let reqData = res.data.data;
                if (status == 0) {
        ...
        childInsertData(parentModuleId, rowid, subtitle1, key) {
            axios
                .post(this.settingConfig.commonUrl + "/child/insertChildModule", { parentModuleId, rowid, subtitle1, key })
                .then(res => {
                    let status = res.data.status;
                    let reqData = res.data.data;
                    ...
                    if (status == 0) {
                        for(let i=this.pageData.length-1;i>=0;i--){
                            if(this.pageData[i].drillDownKeyCurrent&&this.pageData[i].drillDownKeyCurrent!=key){
                        ...
                        reqData.forEach(items => {
                            ...
                            if(items.contentAreaConfig.isPage=="1"){
                                ...
                                items.data=this.childData.slice(0,items.paginationAll.pageSize)
                                // console.log(items)
                            }
        ...
        addKeep(contentAreaConfig) {
            axios.post(this.settingConfig.commonUrl + "/page/addModule", { contentAreaConfig: contentAreaConfig, menuId: this.menuId }).then(res => {
        ...
        screenKeep(conditionAreaConfig, moduleId) {
            ...
            axios
                .post(this.settingConfig.commonUrl + "/page/emitScreenModule", { conditionAreaConfig, moduleId })
                .then(res => {
                    let status = res.data.status;
                    // let reqData = res.data.data;
                    if (status == 0) {
