export const dataMixins = {
    ...
    methods: {
        ...
        //图表宽高设置
        setDemos() {
            this.modelStyle.height = parseFloat(this.settingForm.height * this.mainStyle.height / 100);
            this.modelStyle.width = parseFloat(this.settingForm.width * this.mainStyle.width / 100);
            this.modelStyle.top = parseFloat(this.settingForm.top * this.mainStyle.height / 100);
            this.modelStyle.left = parseFloat(this.settingForm.left * this.mainStyle.width / 100);

        },
        //获取模块初始位置和宽高
        getDemos() {
            this.settingForm.height = this.modelStyle.height / this.mainStyle.height * 100;
            this.settingForm.width = this.modelStyle.width / this.mainStyle.width * 100;
...
export const childMixins = {
    data() {
        return {
            ...
            childSettingForm: {
                ...
                url: "/api/kftjData", //接口
                keyArr: [
                    { key: "title", explain: "类型", dw: "",relationKey:"", width: 180 },
                    { key: "num", explain: "个数", dw: "个",relationKey:"", width: 100 },
                    { key: "area", explain: "面积", dw: "公顷",relationKey:"", width: 120 }
                ],
                height: 24.55,
                width: 27.69,
                top: 32.02,
    ...
    methods: {
        //表格行点击事件
        rowClick(rowData) {
            if (this.settingForm.submodule != "1" || this.settingForm.clickToShow != "row") return;
            ...
            if (rowData.url) {
                ...
                if (this.statisticsAll.childModuleId) {
                    this.$emit("childInsertData", this.statisticsAll.moduleId, rowData.id, subtitle1)
        ...
        cellClick(rowData, key) {
            if (this.settingForm.submodule != "1" || this.settingForm.clickToShow != "cell") return;
            this.nowRowId = rowData.id;
            this.nowCellKey = key;
            ...
            if (rowData[key + 'url']) {
                ...
                if (this.statisticsAll.childModuleId && this.statisticsAll.drillDownKeyAll && this.statisticsAll.drillDownKeyAll.indexOf(key) > -1) {
                    this.$emit("cellClick",rowData,this.statisticsAll)
                    let subtitle1 = rowData[this.settingForm.keyArr[0].key]
                    this.$emit("childInsertData", this.statisticsAll.moduleId, rowData.id, subtitle1, key)
                } else {
                    if (this.isAdmin) {
                        this.$refs['childSettingForm'].show()
                        this.childAddType='1'
...
export const screenMixins = {
    ...
    methods: {
        //筛选模块配置图标点击事件
        screenSetting() {
            this.$refs['screenSetting'].show(this.statisticsAll.conditionAreaConfig, this.whereOffon)
        },
        //筛选模块配置数据保存事件
        ...
        filterShow() {
            ...
            this.$refs['where'].show(this.statisticsAll.conditionAreaConfig, this.whereOffon)
        },
        //内容部分点击事件
